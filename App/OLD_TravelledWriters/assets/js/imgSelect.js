var imgSelect = {
	...
	php: '/imgselect',  //Ajax url
	alert: '.imgSelect-alert', //Response selector
	setImg: '.img',            //Image src to be replaced
	crop: {
		...
		//minHeight: 200,	 // Minimum selection height (in pixels)
		//minWidth: 200,	 // Minimum selection width (in pixels)
	},
	...
	upload: function() {
		...
		new AjaxUpload( $('.ajaxupload') , {
			action: '/imgselect',
			data: {'action': 'upload', area:$('#photo_upload_action').val()},
			name: 'ajax-uploadimage',
			responseType: 'json',
			onSubmit: function(file, ext) {
				imgSelect.removeSelection();
                $('#photo_uploader .ajaxupload').addClass('loading-blue');
			},
			onComplete: function(img, data) {
				if (data.success) {
					...
					imgSelect.setSelection( $('.imgSelect-upload img') );
                    $('.ajaxupload').removeClass('loading-blue');
			...
			onChange: function() {
				imgSelect.cancel();
				$('#photo_uploader .imgSelect-container, .imgSelect-upload').show();
			}
		});
	},
	...
	save: function() {
		...
		if (img.length) {
			...
			data = {action:'save', area:$('#photo_upload_action').val(), 'x1': x1, 'y1': y1, 'w': w, 'h': h};
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
                    $('#photo_uploader .save-link').addClass('loading-green');

                    $('.imgSelect-actions .save').prop('disabled', true);
				},
				success: function(data) {
					if (data.success) {
                        $(imgSelect.setImg).attr('src', data.data.show_photo);
                        $('#mini-photo img').attr('src', data.data.show_photo);
                        if ($('#photo_upload_action').val() == 'story') {
                            $('#story_photo').val(data.data.profile_photo);
                        }

						imgSelect.cancel();

                        $('#photo_uploader .save-link').removeClass('loading-green');

                        $('#photo_uploader .profile_left').css('margin-right', '0px');
	...
	setSelection: function(img) {
		...
		img.load(function(){
			...
			y2 = y1 + crop.height;
            $('#photo_uploader .profile_left').css('margin-right', '20px');
            $.fn.colorbox.resize({
                width:  w + 277 + "px",
                height: h + 130 + "px"
            });
